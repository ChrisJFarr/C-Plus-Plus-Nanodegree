...
ChatLogic::ChatLogic()
{
    //// STUDENT CODE
    ////
    // Now this is done in function ChatLogic::LoadAnswerGraphFromFile, should this remain?
...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes  // No need to delete since now smart pointers
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // Must pass unique-ptr as a referencce
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            _nodes.emplace_back(std::make_unique<GraphNode>(id));  // new
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // Make unique pointer here, defining ownership strategy
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);
                            edge->SetChildNode(childNode->get());
                            edge->SetParentNode(parentNode->get());
                            _edges.push_back(edge.get());  // Pass raw pointer to edges
    ...
    // std::unique_ptr<GraphNode> rootNode = nullptr;  // Can you assign nullptr to smart pointers?
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            if (rootNode == nullptr)
            {
                ...
                // TODO how should this be adjusted when using smart pointers?
            }
            else
            {
